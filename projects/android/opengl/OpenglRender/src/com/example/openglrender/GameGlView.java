...
public class GameGlView extends GLSurfaceView {
	...
	public GameGlView(Context context, boolean translucent, int depth,
			int stencil) {
		super(context);
		Log.d(TAG, "GameGlView create--------------------");
	...
	private void init(boolean translucent, int depth, int stencil) {
		 ...
		 * definition below
		 */
		Log.d(TAG, "init step1--------------------");
		setEGLContextFactory(new ContextFactory());
		Log.d(TAG, "init step2--------------------");
		/*
	...
	private static class ConfigChooser implements
		...
		public EGLConfig chooseConfig(EGL10 egl, EGLDisplay display) {
			...
			Log.d(TAG, "NumConfigs = " + numConfigs);
			if (numConfigs <= 0) {
	...
	private static class Renderer implements GLSurfaceView.Renderer {
		...
		private CFPSMaker fpsMaker = null;
		...
		public void onDrawFrame(GL10 gl) {
			...
			try {
				fpsMaker.makeFPS();
				Log.d(TAG, "FPS:" + fpsMaker.getFPS());
				TestCaseActivity.um_vrend_renderFrame();
		...
		public void onSurfaceChanged(GL10 gl, int width, int height) {
			...
			TestCaseActivity.um_vrend_resize(width, height);
		}

		public void onSurfaceCreated(GL10 gl, EGLConfig config) {
			Log.d(TAG, "onSurfaceCreated--------------------");
			fpsMaker = new CFPSMaker();
			TestCaseActivity.um_vrend_create();
			Log.d(TAG, "onSurfaceCreated-------------------- exit");
		}
	}
}
